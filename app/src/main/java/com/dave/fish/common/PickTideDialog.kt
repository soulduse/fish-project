...
class PickTideDialog : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        val sideSpinner = rootView?.findViewById<CustomAreasSpinner>(R.id.custom_areas_spinner).apply {
            this?.init(Constants.KEY_TIDE_SIDE_SPINNER)
        }

        builder.setView(rootView)
                // Add action buttons
                .setPositiveButton("설정", DialogInterface.OnClickListener { _, _ ->
                    sideSpinner?.getPickedValueOfTide(mListener)
                })
